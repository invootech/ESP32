/*
  Seven Segment Display Test Program
  This program sequentially turns on each segment (A, B, C, D, E, F, G, and DP)
  of a common cathode 7-segment display for testing purposes. The COM pin 
  of the display is connected to GND via pin 19.
...
void loop ( ) {
  for ( int i = 0; i < 8; i++ ) {
    digitalWrite ( SEGMENTS[i], HIGH ); // Turn on the current segment
    delay ( 500 );                      // Wait 500 milliseconds
    digitalWrite ( SEGMENTS[i], LOW );  // Turn off the current segment
  }
}